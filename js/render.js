    ...
    <div class="side-container">
        ...
        <div class="text-editor-container">
            <div class="text-params">
                ...
                <label for="color-choice" onclick="renderColorPallete('fill','Select Text Color')" class="text-edit-btn btn-change-font-color"></label>
        ...
        <form class="flex space-between column"action="" method="POST" enctype="multipart/form-data" onsubmit="onShareMeme(this, event)">
...
function renderColorPallete(kind, header) {
    ...
    const el = document.querySelector('.color-pallete-modal')
    el.classList.toggle('hidden')
    strHtml = `<div class="color-pallete-container"><h3>${header}</h3>`
    colorPallete.forEach(color => {
        strHtml += `<div class="color-choice" style="background-color : ${color}" onclick="onChangeColor(this,'${kind}')"></div>`
    });
    strHtml += '</div>'
    strHtml += `<div class="text-color-input-container">
                    <h3>...Or specify a hex color</h3>
                    <div>
                        <input type="text" class="color-text-input" oninput="onEnterColorText(this)" placeholder = "#FFFFFF" pattern="^#([a-fA-F0-9]){3}$|^#[a-fA-F0-9]{6}$"></input>
                        <div class="color-picker-display" onclick="onChangeColor(this,'${kind}')" style="background-color: #fffff"></div>
                    </div>
                </div>`
